...
wanted_meas = [5,8,11,13,15];
repeats = 8;
qubits = 3;
ranknum = 1;
real = 0;
final_means = zeros(1,15); final_vars = zeros(1,15);

for choice = wanted_meas
    clearvars -except collection means_length ...
        best_fidelity final_means final_vars choice repeats qubits ...
        ranknum real mean_infidelities
    amount_of_randoms = choice;
    if amount_of_randoms == 15
        rounds = 1;
    elseif amount_of_randoms == 14
        rounds = 2;
    else
        rounds = 8;
    end
    for times = 1:rounds
        ...
        for jj = 1:repeats
            ...
            measurements;origior
            [x, fval, history] = fmincon_rand(x0, pauli_new, measurements, selection);
...
figure('name', 'Means w.r.t amount of measurements')
hold on, grid on
...
ylim([0,0.8]);
xlabel('Mean infidelity');
ylabel('Amount of random measurement directions');
